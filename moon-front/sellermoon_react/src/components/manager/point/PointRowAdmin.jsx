  ...
  const pointDelete = (e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) { /* ********알림창에서 삭제 클릭 시 axios 실행********** */
        ...
        axios.get(process.env.REACT_APP_SPRING_IP +"point/pointdelete?point_no="+props.point.POINT_NO)
        .then((response) => {
          console.log(response);
          console.log(response.data);
        ...
        <form id="f_board" onSubmit={pointUpdate}>
          ...
          <Container>
            <Form.Group className="mb-4 mt-3">
              ...
              <Form.Control
                  type="text" name='point_no' id="point_no" size="md" onChange={onChange} 
                  defaultValue={result.POINT_NO} disabled readOnly />
            </Form.Group>

            <Form.Group className="mb-5">
              <Form.Label className="m_title">회원번호</Form.Label>
              <Form.Control
                  type='text' id='member_no' name='member_no' size="md" onChange={onChange} 
                  defaultValue={result.MEMBER_NO} disabled readOnly />
            </Form.Group>
            ...
            <Form.Group className="mb-5">
              ...
              <Form.Select aria-label="Default select example" size="md"
                            id="point_type" name="point_type" onChange={onChange}>
                  <option>내용 선택</option>
...
2                  <option value="3">상품후기등록</option>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                    type='text' id='point_used_saved' name='point_used_saved' size="md" 
                    defaultValue={result.POINT_USED_SAVED} onChange={onChange}  />
            </Form.Group>
          </Container>
