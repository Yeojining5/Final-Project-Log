  ...
  const faqDelete = (e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) { /* ********알림창에서 삭제 클릭 시 axios 실행********** */
        axios.get(process.env.REACT_APP_SPRING_IP +"faq/faqdelete?faq_no="+props.faq.FAQ_NO)
        .then((response) => {
      ...
      <tr>
        ...
        <td>
          <div className="d-flex" style={{margin: "auto"}}>
            <Button onClick={()=>{ navigate('/admin/faq/update/'+result.FAQ_NO)}}
            ...
            <Button variant="outline-secondary" id="btn_search" style={{ marginLeft: "20px", width:"100px"}}
                    onClick={faqDelete}> <i className="fa-regular fa-trash-can"></i>
                &nbsp;삭제
            </Button>
            {/* *********************************************************************************** */}
          </div>
